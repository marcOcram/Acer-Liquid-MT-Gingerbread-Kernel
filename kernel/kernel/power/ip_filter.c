...
int get_udp_table(packet_item *ptable, int *pcount)
{
	...
	ACER_DBG("Get_UDP_Table!\n");
	{
		...
		for (i = 0; i < UDP_HTABLE_SIZE_MIN; i++) {
			struct sock *sk = NULL;
			...
			if (si != NULL) {
				if (count < MAX_ALLOW_TABLE_SIZE) {
					...
					ptable[count].ip_local = ntohl(si->inet_saddr);
					ptable[count].port_local = ntohs(si->inet_sport);
					ptable[count].ip_foreign = ntohl(si->inet_daddr);
					ptable[count].port_foreign = ntohs(si->inet_dport);
