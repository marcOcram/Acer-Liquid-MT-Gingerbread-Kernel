...
#include "mt9e013.h"
#if defined(CONFIG_MACH_ACER_A5) && defined(CONFIG_ADP1650_FLASH_DRIVER)
#include "led_adp1650.h"
#endif
#if defined(CONFIG_MACH_ACER_A5)
uint16_t mt9e013_alight_table[OTP_SHADING_SIZE];
uint16_t mt9e013_cwf_table[OTP_SHADING_SIZE];
uint16_t mt9e013_d65_table[OTP_SHADING_SIZE];
uint16_t mt9e013_gain_table[OTP_GAIN_SIZE]; // 0:RG, 1:GG, 2:BG
uint16_t rolloff_type = 0;
#endif
/*=============================================================
...
#if defined(CONFIG_MACH_ACER_A5)
static int mt9e013_write_lsc_to_sensor(uint16_t mode, uint16_t type)
{
	uint16_t write_addr[7] = {0x3600, 0x3640, 0x3680, 0x36C0, 0x3700, 0x3782, 0x37C0};
	uint16_t write_size[7] = {20, 20, 20, 20, 20, 2, 4};
	uint16_t i = 0, j = 0, write_num = 7;
	uint16_t value = 0;
	uint16_t *ptable = NULL;

	CDBG("%s: %d, mode = %d, rolloff type = %d\n", __func__, __LINE__, mode, type);
	if (mode == RES_PREVIEW) {
		//In preview mode, always apply CWF table
		ptable = mt9e013_cwf_table;
		CDBG("Preview apply CWF table to sensor.\n");
	} else { //RES_CAPTURE
		switch (type) {
		case 0: //ISP3A_ROLLOFF_TL84_LIGHT (defined in Isp3a_chormatix.h)
			ptable = mt9e013_cwf_table;
			CDBG("Capture apply CWF table to sensor.\n");
			break;
		case 1: //ISP3A_ROLLOFF_A_LIGHT
			ptable = mt9e013_alight_table;
			CDBG("Capture apply a light table to sensor.\n");
			break;
		case 2: //ISP3A_ROLLOFF_D65_LIGHT
			ptable = mt9e013_d65_table;
			CDBG("Capture apply D65 table to sensor.\n");
			break;
		}
	}

	if (ptable == NULL || *ptable == 0) {
		CDBG("Don't apply lsc to sensor because null pointer or zero value.\n");
		return 0;
	}
	for (i = 0; i<write_num; i++) {
		for  (j = 0; j<write_size[i]; j++) {
			mt9e013_i2c_write_w_sensor(write_addr[i]+(j*2), *ptable);
			if (i ==0 && j <= 5)
				CDBG("Write lsc value(0x%04x)\n", *ptable);
			ptable++;
		}
	}
	mt9e013_i2c_read(0x3780, &value, 2); //POLY_SC_ENABLE
	mt9e013_i2c_write_w_sensor(0x3780, value | 0x8000);
	return 0;
}

static int mt9e013_otp_load_default(struct mt9e013_default_rolloff const *reg_rolloff_tbl, int num)
{
	int counter = 0;

	CDBG("%s: %d\n", __func__, __LINE__);

	if (reg_rolloff_tbl == NULL) {
		CDBG("otp_load_default, Null pointer.\n");
		return 0;
	}

	for (counter = 0; counter < num; counter++) {
		if (counter < OTP_SHADING_SIZE) {
			mt9e013_alight_table[counter] = reg_rolloff_tbl->alight_data;
			mt9e013_cwf_table[counter] = reg_rolloff_tbl->cwf_data;
			mt9e013_d65_table[counter] = reg_rolloff_tbl->d65_data;
		} else {
			mt9e013_gain_table[counter-OTP_SHADING_SIZE] = reg_rolloff_tbl->cwf_data;
		}
		reg_rolloff_tbl++;
	}
...
static int mt9e013_otp_read(uint16_t type)
{
	static bool load_default = false;
	uint16_t counter = 0, value = 0;
	uint16_t shading_addr_start = 0, gain_addr_start = 0;
	uint16_t *ptable = NULL, *ptable_head = NULL;
	uint8_t block_num = 2, first_block_value = 0;

	CDBG("%s: %d\n", __func__, __LINE__);

	if (load_default) {
		CDBG("Load rolloff default value.\n");
		return 0;
	}

	//Record type
	mt9e013_i2c_write_w_sensor(0x304C, type); //choose to only read reacord type
	//Read command
	...
	switch (type) {
		case OTP_SHADING_A_LIGHT:
			memset(&mt9e013_alight_table, 0, OTP_SHADING_SIZE);
			ptable = mt9e013_alight_table;
			break;
		case OTP_SHADING_CWF:
			memset(&mt9e013_cwf_table, 0, OTP_SHADING_SIZE);
			ptable = mt9e013_cwf_table;
			break;
		case OTP_SHADING_D65:
			memset(&mt9e013_d65_table, 0, OTP_SHADING_SIZE);
			ptable = mt9e013_d65_table;
			break;
	}
	if (ptable == NULL) {
		CDBG("otp_read, Null pointer.\n");
		return 0;
	}

	ptable_head = ptable;
	shading_addr_start = OTP_SHADING_ADDR_START_1;
	gain_addr_start = OTP_GAIN_ADDR_START_1;

	while (block_num) {
		if (block_num == 1) { //second block
			value = 0;
			shading_addr_start = OTP_SHADING_ADDR_START_2;
			gain_addr_start = OTP_GAIN_ADDR_START_2;
			mt9e013_i2c_read(shading_addr_start+(counter*2), &value, 2);
			if (value == 0) { //Second block is no value, so first block is success
				if (first_block_value != 0) {
					CDBG("Read first block success.");
				} else { //Loading default value
					mt9e013_otp_load_default(mt9e013_regs.reg_default_rolloff,
						mt9e013_regs.reg_default_rolloff_size);
					load_default = true;
				}
				return 0;
			} else {
				ptable = ptable_head;
				CDBG("Read first block fail, try to read second block");
			}
		}
		...
		value = 0;
		counter = 0;
		for (counter = 0; counter<OTP_SHADING_SIZE; counter++) {
			mt9e013_i2c_read(shading_addr_start+(counter*2), &value, 2);
			CDBG("counter(%d), Address=0x%04x, value(0x%04x)\n",
			counter, shading_addr_start+(counter*2), value);
			*ptable = value;
			ptable++;
		}
		first_block_value = value;

		//Read gain value for CWF
		if (type == OTP_SHADING_CWF) {
			value = 0;
			counter = 0;
			memset(&mt9e013_gain_table, 0, OTP_GAIN_SIZE);
			for (counter = 0; counter<OTP_GAIN_SIZE; counter++) {
				mt9e013_i2c_read(gain_addr_start+(counter*2), &value, 2);
				CDBG("counter(%d), Address=0x%04x, value(0x%04x)\n",
				counter, gain_addr_start+(counter*2), value);
				mt9e013_gain_table[counter] = value; // 0:RG, 1:GG, 2:BG
			}
		}
		--block_num;
	}
...
static int mt9e013_otp_record_type_read(uint16_t type)
{
	int16_t value = 0;

	CDBG("%s: %d\n", __func__, __LINE__);
	//Initialize OTPM for reading
	mt9e013_i2c_read(0x301A, &value, 2); //RESET_REGISTER_REG_ED_EN
	mt9e013_i2c_write_w_sensor(0x301A, value | 0x0020);
	...
	//Read command
	mt9e013_otp_read(type);
	return 0;
}

static int mt9e013_get_wb_gains_from_module(uint16_t *r_gain, uint16_t *g_gain, uint16_t *b_gain)
{
	*r_gain = mt9e013_gain_table[0];
	*g_gain = mt9e013_gain_table[1];
	*b_gain = mt9e013_gain_table[2];

	CDBG("%s: red_gain(0x%04x), gran_gain(0x%04x), blue_gain(0x%04x)\n",
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	if (update_type == REG_INIT) {
		...
		CSI_CONFIG = 0;
			...
			msleep(100);
#if defined(CONFIG_MACH_ACER_A5)
			mt9e013_write_lsc_to_sensor(rt, rolloff_type);
#endif
		if (rt == RES_PREVIEW) {
...
#if defined(CONFIG_MACH_ACER_A5) && defined(CONFIG_ADP1650_FLASH_DRIVER)
	/* Disable LED flash driver IC */
	gpio_direction_output(data->flash_data->flash_src->
					_fsrc.drv_ic_src.strobe_gpio, 0);
	gpio_free(data->flash_data->flash_src->
					_fsrc.drv_ic_src.strobe_gpio);
	gpio_direction_output(data->flash_data->flash_src->
					_fsrc.drv_ic_src.flash_en_gpio, 0);
	gpio_free(data->flash_data->flash_src->
					_fsrc.drv_ic_src.flash_en_gpio);
#endif
	return 0;
}
...
#if defined(CONFIG_MACH_ACER_A5)
	for (retryid = 0; retryid < 5; retryid++) {
		rc = mt9e013_i2c_read(0x0000, &chipid, 2);
		if (chipid == 0x4B00) {
			break;
		}
		CDBG("cameraid retry(%d) ID: %x\n", retryid,chipid);
	}
#else
	rc = mt9e013_i2c_read(0x0000, &chipid, 2);
	CDBG("ID: %d\n", chipid);
...
#if defined(CONFIG_MACH_ACER_A5) && defined(CONFIG_ADP1650_FLASH_DRIVER)
	/* Enable LED flash driver IC */
	rc = gpio_request(data->flash_data->flash_src->
				_fsrc.drv_ic_src.strobe_gpio, "mt9e013_storbe");
	if (!rc)
		gpio_direction_output(data->flash_data->flash_src->
				_fsrc.drv_ic_src.strobe_gpio, 0);
	rc = gpio_request(data->flash_data->flash_src->
				_fsrc.drv_ic_src.flash_en_gpio, "mt9e013_flash_en");
	if (!rc)
		gpio_direction_output(data->flash_data->flash_src->
				_fsrc.drv_ic_src.flash_en_gpio, 1);
#endif
...
int mt9e013_sensor_config(void __user *argp)
{
	...
	cdata.cfgtype);
...
#if defined(CONFIG_MACH_ACER_A5)
		case CFG_GET_MODULE_WB_INFO:
			mt9e013_get_wb_gains_from_module(
				&(cdata.module_wb_info.red_gain),
				&(cdata.module_wb_info.green_gain),
				&(cdata.module_wb_info.blue_gain));
		...
		case CFG_SET_MODE:
#if defined(CONFIG_MACH_ACER_A5)
			rolloff_type = cdata.rolloff_type;
			CDBG("Set rolloff table = %d\n", rolloff_type);
#endif
...
#if defined(CONFIG_MACH_ACER_A5) && defined(CONFIG_ADP1650_FLASH_DRIVER)
	/* Disable LED flash driver IC */
	gpio_direction_output(mt9e013_ctrl->sensordata->flash_data->flash_src->
				_fsrc.drv_ic_src.strobe_gpio, 0);
	gpio_free(mt9e013_ctrl->sensordata->flash_data->flash_src->
				_fsrc.drv_ic_src.strobe_gpio);
	gpio_direction_output(mt9e013_ctrl->sensordata->flash_data->flash_src->
				_fsrc.drv_ic_src.flash_en_gpio, 0);
	gpio_free(mt9e013_ctrl->sensordata->flash_data->flash_src->
		...
		struct msm_sensor_ctrl *s)
{
	...
	if (rc < 0) {
		CDBG("%s: led flash driver ic does not work.\n", __func__);
	}
#endif
	msm_camio_clk_rate_set(MT9E013_MASTER_CLK_RATE);
	...
	if (rc < 0)
		goto probe_fail;
#if defined(CONFIG_MACH_ACER_A5)
	CDBG("Start read OTPM data.\n");
	//Read OTP data
	mt9e013_otp_record_type_read(OTP_SHADING_A_LIGHT);
	mt9e013_otp_record_type_read(OTP_SHADING_CWF);
	mt9e013_otp_record_type_read(OTP_SHADING_D65);
	CDBG("Stop read OTPM data.\n");
#endif
