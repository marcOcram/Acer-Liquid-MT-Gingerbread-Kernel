		...
		unsigned long arg)
{
	...
	int sensors = 0;		/*Return status of e-compass*/
	...
	switch (cmd) {
		...
		case ECS_IOCTL_GET_DELAY:
			...
			delay = akmd_delay;
			break;
		/*Return e-compass status*/
		case ECS_IOCTL_GET_SENSORS:
			if (AKM8975_DEBUG_INFO)
				AKMDBG("[akm8975] IOCTL_GET_SENSORS : return value = %d", sensors);
			sensors = (atomic_read(&mv_flag) | atomic_read(&m_flag));
			break;
		default:
			return -ENOTTY;
	}
...
//		goto exit4;
	...
	if (!akm->input_dev) {
		err = -ENOMEM;
		printk(KERN_ERR "AKM8975 akm8975_probe: Failed to allocate input device\n");
		goto exit5;
	}
	/* Setup input device */
	...
	/* x-axis acceleration (720) */
	input_set_abs_params(akm->input_dev, ABS_X, -5760, 5760, 0, 0);
	/* y-axis acceleration (720) */
	input_set_abs_params(akm->input_dev, ABS_Y, -5760, 5760, 0, 0);
	/* z-axis acceleration (720) */
	...
	if (err) {
		printk(KERN_ERR "AKM8975 akm8975_probe: Unable to register input device\n");
		goto exit6;
	}
...
#ifndef akm_suspend
static struct dev_pm_ops akm_device_pm_ops = {
