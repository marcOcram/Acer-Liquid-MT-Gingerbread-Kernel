...
#define OVERLAY_PROC_DIR			"overlay"
#define ANIMATION_ACCELERATION_PROC_ENTRY	"animation_acceleration"

static int g_animation_acceleration = 0;
static struct msm_panel_info *gpinfo = NULL;
static __u32 save_vsync_enable;
static __u32 save_hw_vsync_mode;
...
#if defined(CONFIG_MACH_ACER_A4)
static int animation_acceleration_read_proc(char *page, char **start, off_t offset,
					int count, int *eof, void *data)
{
	return sprintf(page, "%s: %d\n", ANIMATION_ACCELERATION_PROC_ENTRY,
						g_animation_acceleration);
}

static int animation_acceleration_write_proc(struct file *file, const char *buffer,
					unsigned long count, void *data)
{
	...
	if (proto == '0') {
		if (g_animation_acceleration) {
			gpinfo->lcd.vsync_enable = save_vsync_enable;
			gpinfo->lcd.hw_vsync_mode = save_hw_vsync_mode;
			g_animation_acceleration = 0;
		}
	} else {
		if (!g_animation_acceleration) {
			save_vsync_enable = gpinfo->lcd.vsync_enable;
			save_hw_vsync_mode = gpinfo->lcd.hw_vsync_mode;
			gpinfo->lcd.vsync_enable = FALSE;
			gpinfo->lcd.hw_vsync_mode = FALSE;
			g_animation_acceleration = 1;
		}
	}
...
int __init msm_fb_init(void)
{
	int rc = -ENODEV;
#if defined(CONFIG_MACH_ACER_A4)
	struct proc_dir_entry *overlay_dir;
...
#if defined(CONFIG_MACH_ACER_A4)
	overlay_dir = proc_mkdir(OVERLAY_PROC_DIR, NULL);
	if (overlay_dir == NULL) {
	...
	ent = create_proc_entry(ANIMATION_ACCELERATION_PROC_ENTRY,
						S_IRUGO|S_IWUGO, overlay_dir);
	if (ent == NULL) {
		printk(KERN_ERR "Unable to create /proc/%s/%s entry",
			OVERLAY_PROC_DIR, ANIMATION_ACCELERATION_PROC_ENTRY);
		return -ENOMEM;
	}
	ent->read_proc = animation_acceleration_read_proc;
	ent->write_proc = animation_acceleration_write_proc;
#endif
