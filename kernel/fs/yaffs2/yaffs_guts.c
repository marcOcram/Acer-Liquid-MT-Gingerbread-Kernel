...
int yaffs_GutsInitialise(yaffs_Device *dev)
{
	...
	if ((!dev->param.inbandTags && dev->param.isYaffs2 && dev->param.totalBytesPerChunk < 1024) ||
		...
		T(YAFFS_TRACE_ALWAYS,
		  (TSTR
		   ("yaffs: NAND geometry problems: nChunksPerBlock %d, nReservedBlocks %d, internalStartBlock %d internalEndBlock %d"
		    TENDSTR),dev->param.nChunksPerBlock, dev->param.nReservedBlocks, dev->internalStartBlock,dev->internalEndBlock));
#endif
		return YAFFS_FAIL;
	}
